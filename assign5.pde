...
void draw() {
  ...
  case GAME_RUN: // In-Game
    ...
    for (int i = 0; i < cabbageX.length; i++) {
      ...
      if (isHit(playerX, playerY, playerX+SOIL_SIZE, playerY+SOIL_SIZE, cabbageX[i], cabbageY[i], cabbageX[i] + SOIL_SIZE, cabbageY[i] + SOIL_SIZE)) {
        playerHealth ++;
    ...
    for (int i = 0; i < clockX.length; i++) {
      ...
      if (isHit(playerX, playerY, playerX+SOIL_SIZE, playerY+SOIL_SIZE, clockX[i], clockY[i], clockX[i] + SOIL_SIZE, clockY[i] + SOIL_SIZE)) {
        addTime(CLOCK_BONUS_SECONDS);//if player touch clocks, time +15 sec
    ...
    for (int i = 0; i < soldierX.length; i++) {
      ...
      // Requirement #3: Use boolean isHit(...) to detect collision
      if (isHit(playerX, playerY, playerX+SOIL_SIZE, playerY+SOIL_SIZE, soldierX[i], soldierY[i], soldierX[i] + SOIL_SIZE, soldierY[i] + SOIL_SIZE)) {
...
boolean isHit(float ax, float ay, float aw, float ah, float bx, float by, float bw, float bh) {
  return false != bw > ax && bx < aw && bh > ay && by < ah;                // Requirement #3
}

String convertFramesToTimeString(int frames) {	// Requirement #4
  String mm = nf(((frames/60)/60)-1, 2);
  String ss = nf((frames/60)/2, 2);
  return mm +":"+ss;
}

color getTimeTextColor(int frames) {				// Requirement #5
  if (frames >= GAME_INIT_TIMER) return #00ffff;
  if (frames >= GAME_INIT_TIMER/2 && frames < GAME_INIT_TIMER) return #ffffff;
  if (frames >= GAME_INIT_TIMER/4 && frames < GAME_INIT_TIMER/2) return #ffcc00;
  if (frames >= GAME_INIT_TIMER/12 && frames < GAME_INIT_TIMER/4) return #ff6600;
  if (frames >= 0 && frames < GAME_INIT_TIMER/12) return #ff0000;
  return #ffffff;
}
